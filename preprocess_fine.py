...
class OpenCLIPNetwork(nn.Module):
    def __init__(self, config, pretrained_path=None):
        ...
        # for OpenAI models
        model, preprocess = clip.load(self.config.clip_model_type,
...
def _embed_clip_sam_tiles(image, sam_encoder):
    ...
    for mode in ['default', 's', 'm', 'l']:
        ...
        tiles = tiles.to("cuda")
        # print(tiles.shape) # [33, 3, 224, 224]
        with torch.no_grad():
            clip_embed = model.encode_image(tiles) #[37, 512] [20, 512]
...
if __name__ == '__main__':
    ...
    save_folder = os.path.join(dataset_path, 'language_features_fine')
